...
public class Ch3TalkManager : MonoBehaviour
{
    ...
    private List<Ch3ProDialogue> ch3ProDialogue;
    ...
    public Ch3MapManager mapManager; // 맵 매니저 참조
    ...
    {
        Instance = this;
        ch3ProDialogue = new List<Ch3ProDialogue>();
        LoadDialogueFromCSV();
        InitializeCharacterImages();
        mapManager = map.GetComponent<Ch3MapManager>();
    ...
    void Update()
    {
        ...
        if (isActivated && (Input.GetKeyDown(KeyCode.Space) || Input.GetMouseButtonDown(0)) && !isWaitingForPlayer)
        {
            ...
            if (!anyTyping)
            {
                ...
                if (currentDialogueIndex >= ch3ProDialogue.Count)
                {
        ...
        if (isWaitingForPlayer && mapManager != null)
        {
            ...
            if (mapManager.currentState == MapState.Cafe && mapManager.isInCafeBarZone && (currentDialogueIndex == 5 || currentDialogueIndex == 73 || currentDialogueIndex == 146 || currentDialogueIndex == 274 || currentDialogueIndex == 364 || currentDialogueIndex == 409 || currentDialogueIndex == 456 || currentDialogueIndex == 518))
            {
                ...
                cafe.SetActive(true);
                currentDialogueIndex++;
                PrintCh3ProDialogue(currentDialogueIndex);
    ...
    private void HandleDialogueProgression(int index)
    {
        if (index == 7) // 룸서비스 랜덤 3건
        {
            ...
            SceneTransitionManager.Instance.HandleDialogueTransition("Ch3Scene", "CafeScene", 8, 3);
        }
        else if (index == 12) // 에스프레소 1잔 직접 주문
        {
            List<CafeOrder> orders = new List<CafeOrder>();
            orders.Add(new CafeOrder("Espresso"));
            SceneTransitionManager.Instance.HandleDialogueTransition("Ch3Scene", "CafeScene", 13, orders);
        }
        else if (index == 14) // 랜덤 주문 2건
        {
            SceneTransitionManager.Instance.HandleRandomMenuTransition("Ch3Scene", "CafeScene", 15, 2);
        }
        else if (index == 19) // 따아 1잔 직접 주문
        {
            List<CafeOrder> orders = new List<CafeOrder>();
            orders.Add(new CafeOrder("HotAmericano"));
            SceneTransitionManager.Instance.HandleDialogueTransition("Ch3Scene", "CafeScene", 20, orders);
        }
        else if (index == 21) // 랜덤 주문 1건
        {
            SceneTransitionManager.Instance.HandleRandomMenuTransition("Ch3Scene", "CafeScene", 22, 1);
        }
        else if (index == 25) // 아아 1잔 직접 주문
        {
            List<CafeOrder> orders = new List<CafeOrder>();
            orders.Add(new CafeOrder("IceAmericano"));
            SceneTransitionManager.Instance.HandleDialogueTransition("Ch3Scene", "CafeScene", 26, orders);
        }
        ...
        else if (index == 75) // 룸서비스 랜덤 4건
        {
            ...
            SceneTransitionManager.Instance.HandleDialogueTransition("Ch3Scene", "CafeScene", 77, 4);
        }
        else if (index == 78) // 랜덤 주문 1건
        {
            SceneTransitionManager.Instance.HandleRandomMenuTransition("Ch3Scene", "CafeScene", 79, 1);
        }
        else if (index == 80) // 따아 1잔 직접 주문
        {
            List<CafeOrder> orders = new List<CafeOrder>();
            orders.Add(new CafeOrder("HotAmericano"));
            SceneTransitionManager.Instance.HandleDialogueTransition("Ch3Scene", "CafeScene", 81, orders);
        }
        else if (index == 92) // 랜덤 주문 1건
        {
            SceneTransitionManager.Instance.HandleRandomMenuTransition("Ch3Scene", "CafeScene", 93, 1);
        }
        else if (index == 94) // 에스프레소 1잔 직접 주문
        {
            List<CafeOrder> orders = new List<CafeOrder>();
            orders.Add(new CafeOrder("Espresso"));
            SceneTransitionManager.Instance.HandleDialogueTransition("Ch3Scene", "CafeScene", 95, orders);
        }
        else if (index == 109) // 랜덤 주문 2건
        {
            SceneTransitionManager.Instance.HandleRandomMenuTransition("Ch3Scene", "CafeScene", 110, 2);
        }

        else if (index == 148) // 룸서비스 랜덤 3건
        {
            Debug.Log("배달 랜덤 룸서비스 주문 3건");
            SceneTransitionManager.Instance.HandleDialogueTransition("Ch3Scene", "CafeScene", 150, 3);
        }
        else if (index == 151) // 랜덤 주문 2건
        {
            SceneTransitionManager.Instance.HandleRandomMenuTransition("Ch3Scene", "CafeScene", 152, 2);
        }
        else if (index == 153) // 에스프레소 1잔 직접 주문
        {
            List<CafeOrder> orders = new List<CafeOrder>();
            orders.Add(new CafeOrder("Espresso"));
            SceneTransitionManager.Instance.HandleDialogueTransition("Ch3Scene", "CafeScene", 154, orders);
        }
        else if (index == 162) // 랜덤 주문 3건
        {
            SceneTransitionManager.Instance.HandleRandomMenuTransition("Ch3Scene", "CafeScene", 163, 3);
        }
        else if (index == 164) // 아아 1잔 직접 주문
        {
            List<CafeOrder> orders = new List<CafeOrder>();
            orders.Add(new CafeOrder("IceAmericano"));
            SceneTransitionManager.Instance.HandleDialogueTransition("Ch3Scene", "CafeScene", 165, orders);
        }

        else if (index == 276) // 룸서비스 랜덤 3건
        {
            Debug.Log("배달 랜덤 룸서비스 주문 3건");
            SceneTransitionManager.Instance.HandleDialogueTransition("Ch3Scene", "CafeScene", 278, 3);
        }
        else if (index == 279) // 아아 1잔 직접 주문
        {
            List<CafeOrder> orders = new List<CafeOrder>();
            orders.Add(new CafeOrder("IceAmericano"));
            SceneTransitionManager.Instance.HandleDialogueTransition("Ch3Scene", "CafeScene", 280, orders);
        }
        else if (index == 305) // 랜덤 주문 4건
        {
            SceneTransitionManager.Instance.HandleRandomMenuTransition("Ch3Scene", "CafeScene", 306, 4);
        }
        else if (index == 307) // 에스프레소 1잔 직접 주문
        {
            List<CafeOrder> orders = new List<CafeOrder>();
            orders.Add(new CafeOrder("Espresso"));
            SceneTransitionManager.Instance.HandleDialogueTransition("Ch3Scene", "CafeScene", 308, orders);
        }
        else if (index == 332) // 랜덤 주문 1건
        {
            SceneTransitionManager.Instance.HandleRandomMenuTransition("Ch3Scene", "CafeScene", 333, 1);
        }

        else if (index == 366) // 룸서비스 랜덤 3건
        {
            Debug.Log("배달 랜덤 룸서비스 주문 3건");
            SceneTransitionManager.Instance.HandleDialogueTransition("Ch3Scene", "CafeScene", 368, 3);
        }
        else if (index == 369) // 랜덤 주문 5건
        {
            SceneTransitionManager.Instance.HandleRandomMenuTransition("Ch3Scene", "CafeScene", 370, 5);
        }
        else if (index == 371) // 따아 1잔 직접 주문
        {
            List<CafeOrder> orders = new List<CafeOrder>();
            orders.Add(new CafeOrder("HotAmericano"));
            SceneTransitionManager.Instance.HandleDialogueTransition("Ch3Scene", "CafeScene", 372, orders);
        }
        else if (index == 402) // 아아 1잔 직접 주문
        {
            List<CafeOrder> orders = new List<CafeOrder>();
            orders.Add(new CafeOrder("IceAmericano"));
            SceneTransitionManager.Instance.HandleDialogueTransition("Ch3Scene", "CafeScene", 403, orders);
        }

        else if (index == 411) // 룸서비스 랜덤 5건
        {
            Debug.Log("배달 랜덤 룸서비스 주문 5건");
            SceneTransitionManager.Instance.HandleDialogueTransition("Ch3Scene", "CafeScene", 413, 5);
        }
        else if (index == 414) // 랜덤 주문 4건
        {
            SceneTransitionManager.Instance.HandleRandomMenuTransition("Ch3Scene", "CafeScene", 415, 4);
        }
        else if (index == 416) // 에스프레소 1잔 직접 주문
        {
            List<CafeOrder> orders = new List<CafeOrder>();
            orders.Add(new CafeOrder("Espresso"));
            SceneTransitionManager.Instance.HandleDialogueTransition("Ch3Scene", "CafeScene", 417, orders);
        }

        else
        {
            // 기본 대화 진행
            PrintCh3ProDialogue(index);
        }
    }
    ...
    {
        List<Dictionary<string, object>> data_Dialog = Ch0CSVReader.Read("Travel Around The World - CH3");
        ...
        foreach (var row in data_Dialog)
        {
            ...
            ch3ProDialogue.Add(new Ch3ProDialogue(day, location, speaker, line, screenEffect, backgroundMusic, expression, note, quest, questContent));
    ...
    public void PrintCh3ProDialogue(int index)
    {
        if (index == 533)
        {
        ...
        Ch3ProDialogue currentDialogue = ch3ProDialogue[index];
        ...
        else if (currentDialogueIndex == 518)
        {
            mapManager.currentState = MapState.Cafe;
            trainRoom.SetActive(true);
            cafe.SetActive(true);
    ...
    public void ActivateTalk(string locationName, int curDialogueIdx)
    {
        ...
        currentDialogueIndex = ch3ProDialogue.FindIndex(dialogue => dialogue.location == locationName);
